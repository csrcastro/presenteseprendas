...
export default function Share({ image }) {
  return !useHydrated() ? null : (
    <div className="shadow-xs grid h-12 grid-cols-4 gap-0 rounded-sm">
      <FacebookShareButton
        className="flex items-center justify-center  rounded-l-sm bg-[#1877F2]"
      ...
      <WhatsappShareButton
        className="-ml-px flex items-center justify-center rounded-r-sm bg-[#25D366]"
        resetButtonStyle={false}
        url={window.location.href}
      >
